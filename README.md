...
This repo benchmarks 9 popular MCP servers, finds a **25× efficiency spread** between the best and worst, and includes a redesign of the worst offender (Notion, 17,161 tokens) that cuts it to 773 tokens (**95.5% reduction**) with the same capability surface.
...
Output: per-tool and total token counts (o200k tokenizer) for the server's `tools/list` response, serialized as `{name, description, inputSchema}` — the static cost the server adds to every request.
